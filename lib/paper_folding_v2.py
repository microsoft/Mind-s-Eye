...
def get_font(size=18):
    ...
    if size > 1:
        try:
            _font_cache = ImageFont.truetype(r"D:\Perception_dataset\DMSS\arial.ttf", size=size)
        except OSError:
            raise Exception("File not found")
            # _font_cache = ImageFont.load_default()
    return _font_cache

def create_regular_polygon(sides, center=(60, 60), radius=50):
    """Return a list of (x, y) points for a regular polygon with the given number of sides."""
    angle_offset = math.pi / 2  # Align one vertex to top
    return [
        (
...
def draw_paper(draw, poly):
    """Draw the polygonal paper with outline."""
    draw.polygon(poly, fill="white", outline="black")
...
def generate_wrong_options(holes):
    ...
    # Option 2: Mirror transformation (point reflection across (60, 60))
    option2 = [(120 - x, 120 - y) for x, y in holes]

    # Option 3: Random positions within 120x120 grid
    option3 = holes.copy()
    if len(option3) > 1:
            for _ in range(len(option3) - 2):
                option3.pop(random.randrange(len(option3)))
    ...
    # If mirror (option2) is same as original, try rotation
...
def generate_test_image(folds, paper_size,test_number, num_folds, num_holes,OUTPUT_DIR):
    """Generate a test image with optimized processing."""
    global process_images
    process_images = []

    font_bigger = get_font(20)

    initial_poly  = create_regular_polygon(sides=paper_size)  # Hexagon
 # [(10, 10), (110, 10), (110, 110), (10, 110)]
    ...
    img_final = Image.new("RGB", (canvas_size, canvas_size), "white")
    ...
    img_correct = Image.new("RGB", (canvas_size, canvas_size), "white")
    draw_correct = ImageDraw.Draw(img_correct)
    draw_paper(draw_correct,initial_poly)
    ...
    for wrong_holes in wrong_options:
        ...
        img_wrong = Image.new("RGB", (canvas_size, canvas_size), "white")
        draw_wrong = ImageDraw.Draw(img_wrong)
        draw_paper(draw_wrong,initial_poly)
    ...
    total_img.save(out_path,dpi=(300,300))

    return out_path, correct_label # wrong_options[1]
...
def generate_test_image_v2(folds, paper_size,test_number, num_folds, num_holes,OUTPUT_DIR):
    """Generate a test image with optimized processing."""
    global process_images
    process_images = []
 ...
 # [(10, 10), (110, 10), (110, 110), (10, 110)]
    ...
    for idx, wrong_holes in enumerate(wrong_options):
        ...
        draw_holes(draw_wrong, wrong_holes)
        if idx == 1:
            candidate_images.append(("same", img_wrong))
        else:
            candidate_images.append(("wrong", img_wrong))
    ...
    return out_path, correct_label,same_label
